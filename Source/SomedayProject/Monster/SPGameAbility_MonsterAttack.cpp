	...
	const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	...
	const float EffectiveRange = AttackRangeOverride > 0.0f ? AttackRangeOverride : MonsterCharacter->GetAttackRange();
	const float EffectiveRadius = AttackRadiusOverride > 0.0f ? AttackRadiusOverride : MonsterCharacter->GetAttackRadius();
	const float DamageAmount = FMath::Max(MonsterCharacter->GetAttackPower(), BaseDamage);
